...
fn numpy_to_tensor<B: Backend, const D: usize>(numpy_data: NpyData<f32>) -> Tensor<B, D> {
    ...
    let shape: Shape<D> = v[0..D].into_iter().map(|&v| v as usize).collect::<Vec<_>>().into();
...
fn load_multihead_self_attention<B: Backend>(path: &str) -> Result<MultiHeadSelfAttention<B>, Box<dyn Error>> {
...
fn load_multihead_cross_attention<B: Backend>(path: &str) -> Result<MultiHeadCrossAttention<B>, Box<dyn Error>> {
    let query = load_linear(&format!("{}/{}", path, "query"))?;
    let key = load_linear(&format!("{}/{}", path, "key"))?;
    let value = load_linear(&format!("{}/{}", path, "value"))?;
    let out = load_linear(&format!("{}/{}", path, "out"))?;
...
fn load_residual_encoder_attention_block<B: Backend>(path: &str) -> Result<ResidualEncoderAttentionBlock<B>, Box<dyn Error>> {
...
fn load_residual_decoder_attention_block<B: Backend>(path: &str) -> Result<ResidualDecoderAttentionBlock<B>, Box<dyn Error>> {
    let attn = load_multihead_self_attention(&format!("{}/{}", path, "attn"))?;
    let attn_ln = load_layer_norm(&format!("{}/{}", path, "attn_ln"))?;
...
fn load_audio_encoder<B: Backend>(path: &str) -> Result<(AudioEncoder<B>, AudioEncoderConfig), Box<dyn Error>> {
    let n_mels = load_usize::<B>("n_mels", path)?;
    let n_audio_state = load_usize::<B>("n_audio_state", path)?;
    
    let conv1_config = Conv1dConfig::new(n_mels, n_audio_state, 3).with_padding(PaddingConfig1d::Explicit(1));
    let conv2_config = Conv1dConfig::new(n_audio_state, n_audio_state, 3).with_padding(PaddingConfig1d::Explicit(1)).with_stride(2);
    ...
    let blocks: Vec<ResidualEncoderAttentionBlock<B>> = (0..n_layer).map(|i| {
        load_residual_encoder_attention_block(&format!("{}/block_{}", path, i))
    }).collect::<Result<_, _>>()?;
...
fn load_text_decoder<B: Backend>(path: &str) -> Result<(TextDecoder<B>, TextDecoderConfig), Box<dyn Error>> {

    let token_embedding = load_tensor::<B, 2>("token_embedding/weight", path)?;
    let positional_embedding = load_tensor::<B, 2>("positional_embedding", path)?;

    let n_layer = load_usize::<B>("n_layer", path)?; 
    let blocks: Vec<ResidualDecoderAttentionBlock<B>> = (0..n_layer).map(|i| {
        load_residual_decoder_attention_block(&format!("{}/block_{}", path, i))
    }).collect::<Result<_, _>>()?;
